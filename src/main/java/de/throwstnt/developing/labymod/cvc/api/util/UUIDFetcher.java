...
public class UUIDFetcher {
	...
	/**
	 * Checks if this uuid is actually a player
	 * @param uuid
	 * @return
	 */
	public static boolean isValidPlayer(UUID uuid) {
		try {
			getNameWithoutCatch(uuid);
	...
	public static String getNameWithoutCatch(UUID uuid) throws MalformedURLException, IOException {
		if (nameCache.containsKey(uuid)) {
			return nameCache.get(uuid);
		}
		HttpURLConnection connection = (HttpURLConnection) new URL(String.format(NAME_URL, UUIDTypeAdapter.fromUUID(uuid))).openConnection();
